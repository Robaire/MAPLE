...
class HumanInterface(object):
    ...
    def run_interface(
        self, sensor_data, semantic_data, vehicle_data, imu_data, refresh=True
    ):
        """
        Run the GUI
        """
        ...
        info_text = [
            ...
            % (
                str(
                    datetime.timedelta(
                        seconds=int(self._controller.agent._mission_time)
                    )
                )
            ),
            "",
            "IMU data:",
            "Accelerometer:   %4.1f   %4.1f   %4.1f"
            % (imu_data[0], imu_data[1], imu_data[2]),
            "Gyroscope:       %4.1f   %4.1f   %4.1f"
            % (imu_data[3], imu_data[4], imu_data[5]),
            "Vehicle control:",
            " Linear speed:      %5.2f   m/s" % (self._controller.linear_target_speed),
            " Angular speed:     %5.2f   rad/s"
            % (self._controller.angular_target_speed),
            "",
            "Components   Current  Target",
            " Front Drum:   %4.2f    %4.2f  rad/s"
            % (
                vehicle_data.front_drums_speed,
                self._controller.front_drums_target_speed,
            ),
            ...
            " Radiator:     %4.2f   %6s"
            % (abs(vehicle_data.radiator_cover_angle), radiator_state),
            "",
            "Sensors:              %1s    %1s" % ("C", "L"),
            "%s(1)Front:           %3s  %3d%%"
            ...
            "",
            "Status:",
            " Current power:      %7.0f Wh"
            % (self._controller.agent.get_current_power()),
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pykeys.K_TAB:
                    self._sensors_status[self._sensor_active][
                        0
                    ] = not self._sensors_status[self._sensor_active][0]
                    self.agent.set_camera_state(
                        self._sensor_active,
                        self._sensors_status[self._sensor_active][0],
                    )
        ...
        if keys[pykeys.K_UP] or keys[pykeys.K_w]:
            self.linear_target_speed = min(
                self.linear_target_speed + self._linear_speed_increase,
        ...
        if keys[pykeys.K_RIGHT] or keys[pykeys.K_d]:
            self.angular_target_speed = max(
                self.angular_target_speed - self._angular_speed_increase,
        ...
        if keys[pykeys.K_f]:
            self.front_drums_target_speed = min(
                self.front_drums_target_speed + self._drum_speed_increase,
                self._max_drum_speed,
            )
            self.agent.set_front_drums_target_speed(self.front_drums_target_speed)
        ...
        if keys[pykeys.K_g]:
            self.front_arm_target_angle = min(
                self.front_arm_target_angle + self._arm_angle_increase,
                self._max_arm_angle,
            )
            self.agent.set_front_arm_angle(self.front_arm_target_angle)
        ...
        if keys[pykeys.K_h]:
            self.back_arm_target_angle = min(
                self.back_arm_target_angle + self._arm_angle_increase,
                self._max_arm_angle,
            )
            self.agent.set_back_arm_angle(self.back_arm_target_angle)
        ...
        if keys[pykeys.K_j]:
            self.back_drums_target_speed = min(
                self.back_drums_target_speed + self._drum_speed_increase,
                self._max_drum_speed,
            )
            self.agent.set_back_drums_target_speed(self.back_drums_target_speed)
        ...
        return (
            ...
            carla.VehicleVelocityControl(
                self.linear_target_speed, self.angular_target_speed
            ),
            self._sensor_active,
        )
...
class HumanAgent(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        self._hic = HumanInterface(
            self._width, self._height, self._controller, self._scale
        )
        ...
        self.frame = 0
        self.apriltag_estimator = ApriltagEstimator(self)
        self.inertial_estimator = InertialApriltagEstimator(self)

        self.data = pd.DataFrame(
            columns=["frame", "ground_truth", "apriltag", "inertial_apriltag"],
        )
    ...
    def run_step(self, input_data):
        ...
        # Check that the frame has camera data
        # TODO: If we only sample on frames where we can see the lander we will never get poses
        # from the inertial estimator since it will just reset to the apriltag in that instance
        # We need to sample frequently to see how it drifts from the ground truth in between
        # apriltag detections

        # Get pose data
        ground_truth = carla_to_pytransform(self.get_transform())
        inertial_pose = self.inertial_estimator(input_data)  # Must run every frame
        apriltag_pose = self.apriltag_estimator(input_data)

        # Add pose information to the data frame?
        self.data.loc[len(self.data)] = [
            self.frame,
            pytransform_to_tuple(ground_truth),
            pytransform_to_tuple(apriltag_pose),
            pytransform_to_tuple(inertial_pose),
        ]
        ...
        if pose is not None:
            ...
            print(
                f"True: ({true_pose[0]:0.2f}, {true_pose[1]:0.2f}, {true_pose[2]:0.2f})"
            )
            print(f"Est:  ({est_pose[0]:0.2f}, {est_pose[1]:0.2f}, {est_pose[2]:0.2f})")

        self._clock.tick_busy_loop(20)

        quit_, control, active_camera = self._controller.parse_events(
            self._delta_seconds
        )
        if quit_:
