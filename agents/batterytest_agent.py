...
class DummyAgent(AutonomousAgent):
    ...
    def run_step(self, input_data):
        ...
        if mission_time > 3 and mission_time <= 10:
            control = carla.VehicleVelocityControl(1.)
        elif mission_time > 10 and mission_time <= 17:
            control = carla.VehicleVelocityControl(-1.)
        if mission_time > 17 and mission_time <= 24:
            control = carla.VehicleVelocityControl(1.)
        elif mission_time > 24 and mission_time <= 31:
            control = carla.VehicleVelocityControl(-1.)
        if mission_time > 31 and mission_time <= 38:
            control = carla.VehicleVelocityControl(0,-1.)
        if mission_time > 38 and mission_time <= 45:
            control = carla.VehicleVelocityControl(0,1.)
        #if mission_time > 45 and mission_times <= end_time:


        elif mission_time > end_time:
            self.mission_complete()

        self.powers.append(self.get_current_power()) # Power in Watt-Hours
    ...
    def finalize(self):
        ...
        with open('data_output.csv', mode='w') as data_output:
            ...
            data_writer.writerow(['Time', 'Actual', 'Estimated', 'IMU data','Power'])
            for i in range(len(self.times)):
                data_writer.writerow([self.times[i], self.gt_arr[i], self.ia_estimates[i], self.imu_data_arr[i],self.powers[i]])
