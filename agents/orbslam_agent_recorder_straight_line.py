...
class ORBSLAMRecorderAgent(AutonomousAgent):
    ...
    def run_step_unsafe(self, input_data):
        ...
        if self.frame < 50:
            ...
            # ORB-SLAM is disabled for now, use simulated pose
            print(f"Frame {self.frame}: ORB-SLAM disabled, using simulated pose")
            estimate = self.init_pose.copy()
            estimate[0, 3] = self.frame * 0.01  # Move 1cm per frame in X direction
            estimate[1, 3] = 0.0  # Stay on Y=0 line
        ...
        # Navigation control - simplified for straight line
        if self.is_stuck:
