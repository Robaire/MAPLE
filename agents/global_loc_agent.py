...
class MITAgent(AutonomousAgent):
    ...
    def save_boulder_detections(self, csv_filename='boulder_detections_100.csv'):
        with open(csv_filename, 'a', newline='') as f:
            writer = csv.writer(f)
            # Write a single row with the pose (from get_transform()) in the first column,
            # frame number in the second, and all_boulder_detections in the third.
            writer.writerow([self.get_transform(), self.frame, self.all_boulder_detections])
    ...
    def run_step_unsafe(self, input_data):
        ...
        estimate = self.get_transform()
        is_april_tag_estimate = True
