...
class IMUPositionTracker:
    def __init__(self, initial_position=[-2.263231, -4.454633, 1.404660], initial_orientation_rpy=None, sampling_rate=20):
        """
        ...
        self.gravity = np.array([0, 0, -1.622])  # Lunar gravity in m/s²
    ...
    def plot_trajectory(self, save_path, gt_history):
        ...
        # Add dummy points with labels for the axis legend
        # Create dummy points far outside the actual plot boundaries to represent the axes in the legend
        # These won't be visible but will appear in the legend
        dummy_x = ax.scatter([], [], [], c='r', marker='^', label='X axis')
        dummy_y = ax.scatter([], [], [], c='g', marker='^', label='Y axis')
        dummy_z = ax.scatter([], [], [], c='b', marker='^', label='Z axis')
...
def process_imu_data(imu_data_frames, initial_position, initial_orientation_rpy, sampling_rate=20):
    """
    ...
    Returns:
        IMUPosition = object with the processed trajectory
    """
...
if __name__ == "__main__":
    ...
    # Define the rotation angles to apply (adjust these angles as needed)
    # Each angle is in radians
    rotation_angle_x = np.radians(0)    # Rotation around x-axis (pitch)
    rotation_angle_y = np.radians(0)  # Rotation around y-axis (yaw)
    rotation_angle_z = np.radians(0)    # Rotation around z-axis (roll)

    # Create rotation matrices for rotation around each axis
    # Rotation around x-axis (changes y and z)
    Rx = np.array([
        [1, 0, 0],
    ...
    # Rotation around y-axis (changes x and z)
    Ry = np.array([
    ...
    # Rotation around z-axis (changes x and y)
    Rz = np.array([
    ...
    # Combine all rotations into a single rotation matrix
    # The order matters: here using ZYX order (first Z, then Y, then X)
    R_combined = Rx.dot(Ry.dot(Rz))

    # Apply the rotation to each IMU reading
    rotated_imu_data = []
    for frame in imu_data_frames:
        # Extract acceleration and gyro readings
        # Assuming IMU data format: [acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z, ...]
        acc_original = np.array(frame[0:3])
        gyro_original = np.array(frame[3:6])
        
        # Rotate the acceleration readings
        acc_rotated = R_combined.dot(acc_original)
        
        # Rotate the gyroscope readings
        gyro_rotated = R_combined.dot(gyro_original)
        
        # Reconstruct the frame with rotated values
        rotated_frame = list(acc_rotated) + list(gyro_rotated)
        
        # If there are more values in the original frame, append them
        if len(frame) > 6:
            rotated_frame.extend(frame[6:])
            
        rotated_imu_data.append(rotated_frame)

    # Replace the original IMU data with the rotated data
    imu_data_frames = rotated_imu_data
    ...
    # imu_data_frames = imu_data_frames[:10]
