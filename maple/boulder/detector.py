...
#         self.MIN_AREA = 5  # Minimum area to be considered a boulder
#         self.MAX_AREA = 5000  # Maximum area to be considered a boulder
#         self.MIN_LARGE_AREA = 100  # Minimum area to be considered a large boulder
#         self.MAX_DEPTH_FOR_LARGE = 4  # Maximum depth (meters) for large boulder classification
...
#     # def get_large_boulders(self, min_diameter_m=0.4, min_compactness=0.6, max_depth_std=0.2, max_depth_slope=0.15) -> list[NDArray]:
#     #     """Return large boulders that are either round or consistent solid obelisks."""
#     #     large_boulders = []
#     #     for boulder, area, mask, bottom_pix in zip(self.last_boulders, self.last_areas, self.last_masks, self.last_bottom_pixels):
#     #         boulder_depth = boulder[2, 3]
#     #         if boulder_depth <= self.MAX_DEPTH_FOR_LARGE:
#     #             radius_m = np.sqrt(area / np.pi)
#     #             diameter_m = radius_m * 2
...
#     #                 compactness = self._compute_compactness(mask)
#     #                 depth_std = self._compute_depth_std(self.last_depth_map, mask)
#     #                 bottom_depth = self._get_depth_at_pixel(self.last_depth_map, bottom_pix)
#     #                 centroid_depth = boulder[2, 3]

#     #                 if compactness >= min_compactness:
#     #                     large_boulders.append(boulder)
#     #                 else:
#     #                     # Not very round, but is it a solid vertical obelisk?
#     #                     if depth_std < max_depth_std:
#     #                         if abs(centroid_depth - bottom_depth) < max_depth_slope:
#     #                             large_boulders.append(boulder)
#     #     return large_boulders
    
#     # def _get_depth_at_pixel(self, depth_map, pixel):
